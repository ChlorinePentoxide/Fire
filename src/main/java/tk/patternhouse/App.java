    ...
    private static void singleSourceParseWrite(String goal) throws IOException {
        ...
        for(String che:cache3) {
            if(che.startsWith("$(FIRE_SS_ITER)")) {
                ...
                for(File goalfile:goalfiles) cache4.addElement(iterable.replaceAll("$(FIRE_GOAL_FILE)", goalfile.getName()).replaceAll("$(FIRE_GOAL_NAME)", goalfile.getName().substring(0,goalfile.getName().lastIndexOf('.'))));
            } else {
